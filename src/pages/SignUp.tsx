  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (session) {
      ...
      if (referralCode.trim()) {
        const result = await applyReferralCode(referralCode.trim(), session.user.id, email);
        if (result.valid) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="w-full max-w-md text-center">
          ...
          <p className="mt-4 text-gray-600">We sent a confirmation link to <strong>{email}</strong>. Click it to activate your account.</p>
          <Link to="/login">
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-gray-600">Start automating your marketing in minutes</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-lg border border-gray-200 focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/20 outline-none transition-all uppercase tracking-wider"
                placeholder="e.g. JOHN4X2K"
              />
              {referralCode && (
                <p className="mt-1 text-xs text-green-600">15% off your first month if valid</p>
              )}
            </div>
